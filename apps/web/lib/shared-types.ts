...
export const CodeSchema = z.object({
  ...
  chapter: z.string(),
  dutyRate: z.number().nullable(),
  secondaryRate: z.number().nullable(),
  requiresLicence: z.boolean(),
  requiresInspection: z.boolean(),
  isRestricted: z.boolean(),
  isProhibited: z.boolean(),
  importPolicy: z.string().nullable(),
  inspectionAgency: z.string().nullable(),
...
export const DutyResponseSchema = z.object({
  ...
  lines: z.array(z.object({ label: z.string(), amount: z.number() })),
  totalDuty: z.number(),
  landedCost: z.number(),
});
export type DutyResponse = z.infer<typeof DutyResponseSchema>;

export const ClassifyRequestSchema = z.object({
  description: z.string().min(2),
  country: CountrySchema,
  lang: z.enum(["en", "zh-CN", "hi"]).default("en"),
});
